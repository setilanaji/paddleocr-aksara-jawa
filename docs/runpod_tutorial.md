...
**If `vl_rec_backend="native"` still raises `No Paddle model files were found`** — paddlex's `transformers` engine (the one that reads safetensors) is supported by the model but **not exposed through paddleocr 3.5's `PaddleOCRVL` wrapper**. Confirmed by introspection: `vl_rec_backend` only accepts `['native', 'vllm-server', 'sglang-server', 'fastdeploy-server', 'mlx-vlm-server', 'llama-cpp-server']`. `native` maps to `paddle_dynamic` (needs `.pdmodel`). To reach the `transformers` engine you'd have to bypass `PaddleOCRVL` and call paddlex's lower-level model API, or stand up a vLLM/sglang server with the safetensors export and use the `*-server` backend.
